...
def showUsers():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT users.stonksid, users.uname, games.game_name FROM users LEFT OUTER JOIN games ON "
        "users.fav_gameid=games.gameid")
...
def addUsers():
    ...
    if request.method == "POST":
        ...
        if int(request.form["favgameselect"]) != 0:
            ...
            favgame = None
        cur.execute("INSERT INTO users (stonksid,uname,fav_gameid) VALUES (%s,%s,%s);",
                    (new_pkey, request.form["uname"], favgame,))
        conn.commit()
...
def changeUsers():
    ...
    if request.method == "POST":
        ...
        if int(request.form["favgameselect"]) != 0:
            ...
            favgame = None
        cur.execute(
            f"""UPDATE users SET uname='{request.form["newuname"]}', fav_gameid={favgame} WHERE stonksid={request.form["stonksid"]};""")
        conn.commit()
...
def viewUsers():
    ...
    if request.method == "POST":
        ...
        cur.execute(
            f"""SELECT SUM(games.price) FROM games_owned JOIN games ON games.gameid=games_owned.gameid WHERE stonksid={stonksid} """)
        moneyspent_games = cur.fetchall()[0][0]
        cur.execute(
            f"""SELECT SUM(dlc.price) FROM dlcs_owned JOIN dlc ON dlc.dlc_name=dlcs_owned.dlc_name WHERE stonksid={stonksid} """)
        ...
        cur.execute(
            f"""SELECT stonksid, game_name, playtime_hours FROM playtime_with_text WHERE stonksid={stonksid} """)
        playtime = cur.fetchall()
    return render_template("viewusers.html", moneyspent_games=moneyspent_games, moneyspent_dlcs=moneyspent_dlcs,
                           stonksid=stonksid, playtime=playtime)
...
def deleteUsers():
    ...
    if request.method == "POST":
        ...
        # cur.execute(f"""DELETE * FROM users INNER JOIN games_owned ON users.stonksid=games_owned.stonksid INNER
        # JOIN dlcs_owned ON users.stonksid=dlcs_owned.stonksid INNER JOIN playtime ON
        # users.stonksid=playtime.stonksid WHERE stonksid={request.form["stonksid"]}""")
        conn.commit()
